...
class RichTextView @JvmOverloads constructor(private val mContext: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) : AppCompatTextView(mContext, attrs) {
    ...
    fun clearSpans() {
        ...
        // Get spans
        val spans = mSpannableString?.getSpans(0, mSpannableString!!.length, Any::class.java)
     ...
     * Clears all spans applied to our SpannableString.
     */
    fun <T> removeSpan(clazz: Class<T>) {

        if(hasSpan(mSpannableString!!,clazz)) {

            Log.i("Span :"," "+clazz.canonicalName)
            // Get spans
            val spans = mSpannableString?.getSpans(0, mSpannableString!!.length, clazz)
     ...
     */
    fun <T> hasSpan(spanned: SpannableString, clazz: Class<T>):Boolean {
